...
class EventSim(object):
    ...
    def generate_events(self, new_frame: np.ndarray, t_frame: int) -> np.ndarray:
        """
        Notes:
        ...
        # ------------------
        # Event Generation!
        e_t, e_x, e_y, e_p, e_dvd = event_generation(self.thres_on, self.thres_off, mu, var, self.delta_vd_res, self.t_now, t_frame)
        if e_t.shape[0] > 0:
