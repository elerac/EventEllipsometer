...
def sample_non_c_zero(ep, c_in, sigma_in):
    ...
    X = torch.empty_like(ep, dtype=ep.dtype, device=ep.device)
    ...
    x_max_thres = -1 * torch.sqrt(-4 * ep_neg * c_neg / torch.pow(sigma_in_neg, 2.0))
    ...
    x_truncated = sample_truncated_normal(sample_mean, sample_sigma, sample_inf, x_max_thres)
...
def sample_IG_torch(episilon: torch.Tensor, c: torch.Tensor, sigma: torch.Tensor):
    ...
    delta_t_c_non_zero = sample_non_c_zero(ep_non_zeros, c_non_zeros, sigma_non_zeros).reshape(-1)
...
def sample_truncated_normal(mean: torch.Tensor, scale: torch.Tensor, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    def normal_cdf(v, mu, sigma):
        cdf = 0.5 * (1 + torch.erf((v - mu) * sigma.reciprocal() / math.sqrt(2)))
        return cdf
...
def ig_prob_a_b_NC(a, b, lamb, mu, h: int = 4):
    ...
    for para_single, inter_tensor_single in zip(para, inter_tensors):
        out = out + para_single * inverse_gaussian_pdf(inter_tensor_single, lamb, mu)
    out = out * sub
    return out
...
def event_generation(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x=None, y=None):
    ...
    p_first_on = torch.where(torch.isnan(p_first_on), torch.ones_like(p_first_on, device=p_first_on.device), p_first_on)
    p_first_on = torch.where(c == 0, torch.ones_like(p_first_on, device=p_first_on.device) * 0.5, p_first_on)
    ...
    delta_vd_legacy_new[last_event_positions] = delta_vd_legacy_new[last_event_positions] + delta_vd_last[last_event_positions]
    ...
    if torch.sum(this_event_positions) > 0:
        ...
        y_next = events_y
        t_next = events_t
        e_t, e_x, e_y, e_p, e_delta_vd = event_generation(ep_on_next, ep_off_next, c_next, sigma_next, delta_vd_next, t_next, end_t, x_next, y_next)
