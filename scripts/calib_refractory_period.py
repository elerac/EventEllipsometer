...
def record(
    ...
    i_monitoring = device.get_i_monitoring()
    dead_time = i_monitoring.get_pixel_dead_time()
    # print(f"Dead time:  {dead_time} [us]")
    print(f"table[({bias_diff}, {bias_diff_on}, {bias_diff_off}, {bias_fo}, {bias_hpf}, {bias_refr})] = {dead_time}")

    # Delete the files
    shutil.rmtree(str(filepath_raw), ignore_errors=True)
    shutil.rmtree(str(filepath_bias), ignore_errors=True)
...
def main():
    ...
    bias_fo = 55
    bias_hpf = 0
    bias_refr = np.arange(-20, 236, 1)
    for refr in bias_refr:
        record(None, 0.5, bias_diff, bias_diff_on, bias_diff_off, bias_fo, bias_hpf, refr, None, 0.05, False)
